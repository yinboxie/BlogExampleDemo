...
namespace CSum.Util
{
    ...
    public static partial class Extensions
    {
        ...
        /// <param name="type">类型对象</param>
        /// <param name="inherit">是否搜索类型的继承链以查找描述特性</param>
        /// <returns>返回Table特性描述信息，如不存在则返回类型的全名</returns>
        public static string ToTableName(this Type type, bool inherit = false)
        {
            var desc = type.GetAttribute<TableAttribute>(inherit);
            return desc == null ? type.FullName : desc.Name;
        }

        /// <summary>
        ///     获取成员元数据的Description/DisplayName/Display特性描述信息
        /// </summary>
        /// <param name="member">成员元数据对象</param>
        /// <param name="inherit">是否搜索成员的继承链以查找描述特性</param>
        /// <returns>返回Description/DisplayName/Display特性描述信息，如不存在则返回成员的名称</returns>
        ...
        /// <summary>
        ///     获取类型的Description/DisplayName特性描述信息
        /// </summary>
        /// <param name="type">成员元数据对象</param>
        /// <param name="inherit">是否搜索成员的继承链以查找描述特性</param>
        /// <returns>返回Description/DisplayName特性描述信息，如不存在则返回成员的名称</returns>
        public static string ToDisplayName(this Type type, bool inherit = true)
        {
            var desc = type.GetAttribute<DescriptionAttribute>(inherit);
            if (desc != null) return desc.Description;
        ...
        /// <summary>
        ///     获取成员元数据的Column特性描述信息
        /// </summary>
        /// <param name="member">成员元数据对象</param>
        /// <param name="inherit">是否搜索成员的继承链以查找描述特性</param>
        /// <returns>返回Column特性描述信息，如不存在则返回成员的名称</returns>
        public static string ToColumnName(this MemberInfo member, bool inherit = false)
        {
            var display = member.GetAttribute<ColumnAttribute>(inherit);
            if (display != null) return display.Name;
            return member.Name;
        }
        ...
        {
            if (type.IsGenericTypeDefinition) return baseType.IsGenericAssignableFrom(type);
        ...
        public static DbType ToDbType(this Type pType)
        {
            ...
            map[typeof(DateTime?)] = DbType.DateTime;
            map[typeof(DateTimeOffset?)] = DbType.DateTimeOffset;
            //typeMap[typeof(System.Data.Linq.Binary)] = System.Data.DbType.Binary;
